...
- (void)setupTitleLabel:(NSString *)titleStr{
    ...
    CGFloat top = _imageBetweenTitleSpace > 0 ? contentHeight + _imageBetweenTitleSpace : contentHeight + subViweMargin;
    self.titleLabel.frame = CGRectMake(0, top, width, fontSize);
...
- (void)setupDetailLabel:(NSString *)detailStr{
    ...
    CGFloat top = _titleBetweenDetailsSpace > 0 ? contentHeight + _titleBetweenDetailsSpace : contentHeight + subViweMargin;
    self.detailLabel.frame = CGRectMake(0, top, width, height + 5 + 2);
    self.detailLabel.text = detailStr;
    if (detailStr.length > 0) {
        NSMutableAttributedString *muattr = [[NSMutableAttributedString alloc] initWithString:detailStr];
        NSMutableParagraphStyle *style = [[NSMutableParagraphStyle alloc] init];
        [style setLineSpacing:5];
        [style setAlignment:NSTextAlignmentCenter];
        [muattr addAttributes:@{NSParagraphStyleAttributeName:style , NSFontAttributeName:font} range:NSMakeRange(0, detailStr.length)];
        self.detailLabel.attributedText = muattr;
    }
...
- (void)setupActionBtn:(NSString *)btnTitle target:(id)target action:(SEL)action btnClickBlock:(HPActionTapBlock)btnClickBlock{
    ...
    CGFloat borderWidth = self.actionBtnBorderWidth ? self.actionBtnBorderWidth : 1.0f;
    CGFloat cornerRadius = self.actionBtnCornerRadius ? self.actionBtnCornerRadius : 16.f;
    ...
    CGFloat width = self.actionBtnWidth ? self.actionBtnWidth : kActionBtnWidth;

    CGFloat btnHeight = height;//height;
    CGFloat btnWidth = width; //btnWidth > contentMaxWidth ? contentMaxWidth : btnWidth;
    
    CGFloat top = _detailsBetweenButtonSpace > 0 ? contentHeight + _detailsBetweenButtonSpace : contentHeight + subViweMargin;
    
    self.actionButton.frame = CGRectMake(0, top, btnWidth, btnHeight);
    [self.actionButton setBackgroundImage:self.actionBtnBackGroundImage forState:UIControlStateNormal];
...
- (void)setActionBtnBackGroundImage:(UIImage *)actionBtnBackGroundImage {
    if (_actionBtnBackGroundImage != actionBtnBackGroundImage) {
        _actionBtnBackGroundImage = actionBtnBackGroundImage;
